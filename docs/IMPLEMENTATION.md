...
| 2 | `labauto ansible` — installs Ansible via the lab AMI tool |
| 3 | `git clone` monorepo to `/opt/roboshop-ansible` |
| 4 | If not DB tier: wait for `getent hosts` on dependency DNS names |
| 5 | Write `ansible/group_vars/all.yml` |
| 6 | `ansible-playbook -i localhost, -c local main.yml -e COMPONENT=...` |
...
|----------|-------|-------|
| `ami_id` | `ami-0220d79f3f480ecf5` | Lab AMI with labauto tool |
| `ec2_user` | `ec2-user` | |
| `ec2_password` | `DevOps321` | Sensitive in Terraform |
| SSH | `ec2-user` / `DevOps321` | No key pair; password set in user-data |
| `ansible_repo_url` | `https://github.com/nikkaushal/roboshop-v1.git` | Must be pushed before apply |
